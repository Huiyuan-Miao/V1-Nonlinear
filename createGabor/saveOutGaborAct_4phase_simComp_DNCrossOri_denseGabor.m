...
for fR = 1: length(filterRsps)
    ...
    for F = 1 : length(F0ss)
        ...
        for ori = [8]%[4,8,12,16]
            ...
            saveDir = ['./GaborActivationDivNormCrossOriSupp2_4Phase_simpleComplex_',num2str(ori),'ori_',num2str(iw),'px_denseGabor/',filterRsps{fR}];
            if ~exist(saveDir)
                mkdir(saveDir)
            end
...
% apply divisive normalization 
            for r = [0.01,0.05,0.1,0.25,0.5,0.75,1,1.5,2] %[0.01:0.01:2]
                ...
                h5create([saveDir,'/layer',num2str(count2),'Processed.h5'],'/featuremap',[size(c,1),7250])
